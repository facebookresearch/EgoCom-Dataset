...
filler_words = ["huh", "uh", "uhh", "uhhh", "erm", "er", "um", "umm", "mm", "hmm", "hm", "ah", "ahh", "ugh", "eyyy", "eyy", "ey", "ay", "ayy", "ayyy"] # "oh", "ohh"
filler_words += ["inaudible", "crosstalk"] # Words introduced by rev.com erroneously without [] around them so they miss detection.
contractions = {
...
def process_transcript_data(
    df, # pd.DataFrame with cols ["conversation_id", "startTime", "speaker_id", "endTime", "word"]
    verbose = True,
    ...
    # If a row contains a word with spaces, split into two rows in the dataframe.
    lod = df.to_dict('records') # List of dictionaries
    df = pd.DataFrame([dict(d, **{"word": token}) for d in lod for token in d["word"].split(" ")])
    if verbose:
        print("After splitting words with spaces into seperate rows |", len(df))

    # Replace empty strings with spaces. This is caused by using word.split(" ") when word == " "
    ...
    # Seperate punctuation, numbers, and non-numeric words. Seperates contractions, too.
    # e.g. "1900s." becomes ["1900", "s", "."]
    # e.g. contractions like "they've" ---> ["they", "'", "ve"]
    lod = df.to_dict('records') # List of dictionaries
    df = pd.DataFrame([dict(d, **{"word": token}) for d in lod for token in ([" "] if d["word"] == " " else re.findall(r"[^\W\d_]+|\d+|[^\w\s]", d["word"]))])
    if verbose:
        print("After 1900s. --> [1900, s, .] and they've --> [they, ', ve] |", len(df))

    if replace_numbers_with_words:
        # Change all numbers to word-based versions of the number, e.g. ()
        df['word'] = df['word'].apply(lambda x: num2words(int(x)) if x.isdigit() else x)
        if verbose:
            print("After 1100 --> one thousand, one hundred |", len(df))

        # Seperate numbers with dashes into parts, e.g. "twenty-two" becomes ["twenty", "-", "two"]
        # Also seperate numbers with spaces like "one hundred" --> ["one", "hundred"]
        lod = df.to_dict('records') # List of dictionaries
        df = pd.DataFrame([dict(d, **{"word": token}) for d in lod for token in ([" "] if d["word"] == " " else re.findall(r"\w+|[^\w\s]", d["word"]))])
        if verbose:
...
def df2transcripts(
    ...
    for key in sorted(dfs.keys()):
        ...
        if expand_contractions:
            words = words.apply(lambda x: contractions[x] if x in contractions else x)
...
def _run_thread_job(params):  
    try:
        ...
        ts_h = params['hypothesis'] # An automatically estimated transcription string
        error = wer(ts_t, ts_h)
...
def compute_wer_for_all_videos(ts_dict_hypothesis, ts_dict_truth):
    '''Takes two transcript dictionaries (first is estimated, second is ground truth).
    Each are formatted as {'key1':transcript_str1, 'key2':transcript_str2, ...}
    ...
    print("Average Accuracy (1 - word-error-rate):", np.round(1 - np.mean(results),3))

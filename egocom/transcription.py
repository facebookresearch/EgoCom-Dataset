...
def write_subtitles(
    ...
    while i < len(words) - 1:
        ...
        if speakers is None:                
            ...
            word_str = "\n".join([("Speaker " + str(who[z]) if who[z] > 1 else "Curtis") + ": " + "\n\t".join([" ".join(w[word_idx[z]:word_idx[z+1]][i:min(i+max_words_per_line,j)]) for i in range(0, word_idx[z+1] - word_idx[z], max_words_per_line)]) for z in range(len(who))])
        print(cnt, file=wf)
...
def compute_running_consecutive_idx(lst):
    ...
    and the second list is their associated starting indices. Feel free to zip the two lists together
    as a single list of tuples if you desire.'''
...
def compute_duplicates_mask(df, threshold = 0.1):
    ...
    for offset in [0, -1]:
        # Create a filter to determine if two adjacent words started within threshold seconds.
        intertimes = np.ediff1d(df['startTime'])
        close_start = (abs(np.insert(intertimes, offset, 1)) <= threshold + 1e-6)
        # Create a filter to determine if two adjacent words ended within 0.1 seconds.
        ...
        combined_filter = both if combined_filter is None else combined_filter | both
        
    return combined_filter


def find_which_duplicates_to_remove(df, threshold_seconds = 0.1):
    '''This identifies when the same word is picked up and transcribed by multiple speaker's 
    microphones, even though it was only spoken once, by a single speaker. The additional 
    duplicate words are removed from the pandas.DataFrame containing the transcriptions.
    
    The duplicate with the highest confidence is the word we keep.
    
    It will not remove duplicates spoken by the same speaker - for example "Okay, okay let's go"
    or "trains go choo choo" -- those examples will be kept in the transcriptions because the duplicated
    words "okay" and "choo" belong to the same speaker.
    
    An issue can occur if two speakers both legitimately say, for example, "Okay" within the time threshold 
    (default is 0.1 seconds) of eachother. In this rare case, a legitimate spoken word may be removed.
    
    Returns a list of True/False boolean values with true whenever a word should be removed.'''
    
    if "duplicates" not in df.columns:
        df["duplicates"] = compute_duplicates_mask(df, threshold = threshold_seconds)
        
    remove_mask = []
    prev_word = ""
    c_lst = []
    for c, d, w in [tuple(z) for z in df[["confidence", "duplicates", "word"]].itertuples(index=False)]:
        ...
        if d:
            ...
            remove_mask = remove_mask + [z != max(c_lst) for z in c_lst] + [False]
            c_lst = []
